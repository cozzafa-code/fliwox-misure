    ...
    <section style={S.section}>
      ...
      <div style={S.inputRow}>
        <input
          ...
          style={S.input}
        />
        <button onClick={() => setVal('')} style={S.btnInvia}>Invia</button>
      </div>
      <div style={S.list}>
        {NOTE_MOCK.map((n) => (
      ...
      <button style={S.btnPrimary}>Vedi tutte le note</button>
    </section>
  );
}
...
const S = {
  ...
  input: { flex: 1, background: MC.bgSoft, color: MC.text, border: `1px solid ${MC.border}`, borderRadius: MR.sm, padding: '8px 10px', fontSize: 12, fontFamily: 'inherit', outline: 'none' } as CSSProperties,
  btnInvia: { background: MC.teal, color: '#fff', border: 'none', borderRadius: MR.sm, padding: '0 14px', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'uppercase' as const, letterSpacing: 0.5 } as CSSProperties,
  list: { display: 'flex', flexDirection: 'column' as const, gap: 8, marginBottom: 12 } as CSSProperties,
  note: { background: MC.bgSoft, borderRadius: MR.sm, padding: 8 } as CSSProperties,
