    ...
    <section style={S.section}>
      ...
      <div style={S.list}>
        {ATTIVITA_CORR_MOCK.map((a) => {
          const color = a.stato === 'completato' ? MC.success : a.stato === 'in_corso' ? MC.warning : MC.muted;
          const icon = a.stato === 'completato' ? '✓' : a.stato === 'in_corso' ? '●' : '○';
          return (
            <div key={a.id} style={S.row}>
              <span style={S.titolo}>{a.titolo}</span>
              <span style={S.data}>{a.data ?? '—'}</span>
              <span style={{ ...S.badge, background: color, color: '#fff' }}>{icon}</span>
            </div>
          );
        })}
      </div>
...
const S = {
  ...
  row: {
    display: 'grid',
    gridTemplateColumns: '1fr 80px 22px',
    alignItems: 'center',
    gap: 10,
    padding: '8px 10px',
  ...
  badge: { width: 22, height: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800 } as CSSProperties,
};
